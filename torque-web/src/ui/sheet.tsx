    ...
    <RD.Portal>
      ...
      <RD.Content
        className={cn(
          ...
          side === "left" && "left-0 shadow-[inset_-1px_0_0_0_hsl(var(--hairline))] data-[state=open]:slide-in-from-left data-[state=closed]:slide-out-to-left",
          side === "right" && "right-0",
          "flex flex-col",
          className,
        )}
      ...
      >
        {(title || description) && (
          <div className="flex items-start justify-between gap-4 shadow-hairline-b px-6 py-5">
            <div>
              {title && (
            ...
            <RD.Close className="rounded-sm p-1 text-ink-dim hover:bg-elevated hover:text-ink transition-colors">
              <X className="h-4 w-4" />
            </RD.Close>
          </div>
        )}
