import { Facebook, Headphones, Instagram, MessageCircle } from 'lucide-react'
...
/* ─── Icon-only variant ─── */
function IconOnly({ channel, size = 'md' }: Pick<ChannelBadgeProps, 'channel' | 'size'>) {
...
/* ─── Labeled variant ─── */
function Labeled({ channel }: Pick<ChannelBadgeProps, 'channel'>) {
  const meta = CHANNELS[channel]
  const Icon = meta.icon
...
function Dot({ channel }: Pick<ChannelBadgeProps, 'channel'>) {
  ...
  return <span className={cn('inline-block h-1.5 w-1.5 rounded-full', meta.dot)} />
}
...
export function ChannelBadge({
  ...
  return (
    <span
      ...
      className={cn('inline-flex', muted && 'pointer-events-none opacity-40', className)}
    >
