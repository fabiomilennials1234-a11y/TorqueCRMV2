import { ArrowUpRight, ArrowDownRight, Sparkles, MessageSquare, Columns3, Trophy, Flame, Timer } from "lucide-react";
    ...
    <div className="relative">
      ...
      <div className="mx-auto max-w-[1400px] px-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          ...
          <Card className="lg:col-span-2 clay-surface !rounded-xl !shadow-clay-1 hover:!shadow-clay-2">
            <CardHeader>
              <CardTitle>Pipeline · WhatsApp</CardTitle>
            ...
            <CardBody>
              ...
              <div className="mt-6 space-y-1">
                {hot.map((l) => (
                  <div
                    key={l.id}
                    className="group flex items-center gap-3 -mx-2 rounded-md px-2 py-2 hover:bg-elevated/50 cursor-pointer transition-colors"
                  >
                    ...
                    <div className="font-metric text-sm tabular-nums text-accent">
                      {l.score}
                    </div>
            ...
            <CardFooter className="flex items-center justify-between">
              ...
              <a className="text-ink hover:text-accent flex items-center gap-1" href="#">
          ...
          {/* Live activity — clay */}
          <Card className="clay-surface !rounded-xl !shadow-clay-1 hover:!shadow-clay-2">
            <CardHeader>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-3">
          <ShortcutCard icon={Columns3} title="Pipeline WhatsApp" subtitle="12 cards em Qualificado" to="/pipeline" />
          <ShortcutCard icon={MessageSquare} title="9 conversas não lidas" subtitle="3 aguardando > 10min" to="/inbox" tone="warning" />
          <ShortcutCard icon={Sparkles} title="3 agentes IA ativos" subtitle="Handoff rate 14% · estável" to="/copilot" />
        </div>

        <div className="mt-10 mb-10 flex items-center justify-between shadow-[inset_0_1px_0_0_hsl(var(--hairline))] pt-6 text-2xs text-ink-dim">
          <div className="flex items-center gap-3">
            <Trophy className="h-3 w-3 text-accent" />
            <span className="font-metric">Rafael Bento lidera o mês com R$ 340k em pipeline movimentado</span>
          </div>
          <div className="flex items-center gap-3">
...
function Kpi({
  ...
  return (
    <div className="clay-kpi rounded-xl shadow-clay-1 p-6 transition-shadow duration-200 ease-[var(--ease-out-soft)] hover:shadow-clay-2">
      <div className="text-2xs font-medium uppercase tracking-[0.14em] text-ink-dim">
        {label}
      </div>
      <div className="mt-3 flex items-end justify-between gap-4">
        <div className="flex items-baseline gap-1">
          {currency && <span className="text-ink-dim text-sm font-metric">R$</span>}
          <span className="font-display text-[1.9rem] leading-none tracking-tightest text-ink tabular-nums">
            {display}
          </span>
          {suffix && (
            <span className="pb-1 text-sm text-ink-dim font-metric">{suffix}</span>
          )}
        </div>
      ...
      <div
        className={[
          "mt-2 inline-flex items-center gap-1 text-xs font-metric tabular-nums",
          up ? "text-success" : "text-danger",
        ].join(" ")}
      >
        {up ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
        {(delta * 100).toFixed(1)}%
        <span className="text-ink-dim ml-1">vs semana passada</span>
      </div>
    </div>
  );
}
      ...
      <div className="mt-3 grid grid-cols-6 gap-3">
        {data.map((d) => (
          <div key={d.label}>
            ...
            <div className="mt-0.5 font-metric text-sm text-ink tabular-nums">
              {d.n}
            </div>
          </div>
        ))}
    ...
    <li className="relative pl-6">
      ...
      <div className="flex items-baseline justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {meta && (
            <div className="mt-0.5 text-2xs text-ink-dim">{meta}</div>
          )}
        </div>
        <time className="font-metric text-2xs tabular-nums text-ink-dim">
          {formatRelative(at)}
        </time>
      </div>
    </li>
...
function ShortcutCard({
  ...
  return (
    <a
      href={to}
      className="group relative flex items-center gap-4 clay-surface rounded-xl p-4 shadow-clay-1 transition-all duration-200 ease-[var(--ease-out-soft)] hover:shadow-clay-2 hover:-translate-y-px active:shadow-clay-pressed active:translate-y-0"
    >
      <div className={[
