  ...
  return (
    <div className="flex h-[calc(100vh-56px)]">
      ...
      <aside className="flex w-[340px] shrink-0 flex-col shadow-[inset_-1px_0_0_0_hsl(var(--hairline))]">
        <div className="shrink-0 px-4 pt-4 pb-3">
          ...
          <div className="relative">
            ...
            <Input placeholder="Buscar…" className="pl-9 h-8" />
          </div>
          <div className="mt-3 flex items-center gap-1.5">
            {[
              ...
              <Pill
                key={t.k}
                active={tone === t.k}
                onClick={() => setTone(t.k as typeof tone)}
              >
                {t.label}
                <span className="ml-1 font-metric text-2xs text-ink-dim tabular-nums">
                  {t.n}
                </span>
              </Pill>
            ))}
            ...
            return (
              ...
              >
                <div className="relative shrink-0">
                  <Avatar size="md" fallback={c.name.split(" ").map((n) => n[0]).join("")} />
                  {c.online && (
                    <span className="absolute -right-0.5 -bottom-0.5 h-2.5 w-2.5 rounded-full bg-success ring-2 ring-bg" />
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className={cn("truncate text-sm", c.unread ? "text-ink font-medium" : "text-ink-muted")}>
                      {c.name}
                    </span>
                    <span className="font-metric text-2xs tabular-nums text-ink-dim shrink-0">
                      {formatRelative(c.at)}
                    </span>
                  </div>
                  <div className="mt-0.5 flex items-center justify-between gap-2">
                    <span className={cn("truncate text-xs", c.unread ? "text-ink-muted" : "text-ink-dim")}>
                      {c.preview}
                    </span>
                    {c.unread ? (
                      <span className="shrink-0 rounded-full bg-accent px-1.5 text-[0.625rem] font-metric tabular-nums text-bg">
                        {c.unread}
                      </span>
                    ) : null}
                  </div>
      ...
      <section className="flex flex-1 min-w-0 flex-col">
        <div className="shrink-0 flex items-center gap-3 px-6 py-3 shadow-hairline-b">
          <button className="md:hidden">
            <ChevronLeft className="h-4 w-4 text-ink-muted" />
          </button>
          <Avatar size="md" fallback={activeConv.name.split(" ").map(n => n[0]).join("")} />
          <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-0.5">
            <Button variant="ghost" size="icon"><Phone className="h-4 w-4" strokeWidth={1.75} /></Button>
            <Button variant="ghost" size="icon"><Video className="h-4 w-4" strokeWidth={1.75} /></Button>
            <Button variant="ghost" size="icon"><MoreVertical className="h-4 w-4" strokeWidth={1.75} /></Button>
          </div>
        </div>

        {/* AI Status strip */}
        <div className="shrink-0 flex items-center gap-3 bg-accent/5 px-6 py-2 shadow-hairline-b">
          <Sparkles className="h-3.5 w-3.5 text-accent" />
          <span className="text-xs text-accent font-medium">Mila está conduzindo esta conversa</span>
        ...
        <div className="relative flex-1 min-h-0 overflow-y-auto">
          <div className="absolute inset-0 bg-dot-grid [background-size:24px_24px] opacity-30 pointer-events-none" />
          <div className="relative mx-auto max-w-[720px] px-6 py-6 space-y-5">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="rounded-md bg-elevated px-3 py-2 text-sm shadow-hairline">
                <span className="inline-flex gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-ink-dim animate-pulse" />
                  <span className="h-1.5 w-1.5 rounded-full bg-ink-dim animate-pulse" style={{ animationDelay: "150ms" }} />
                  <span className="h-1.5 w-1.5 rounded-full bg-ink-dim animate-pulse" style={{ animationDelay: "300ms" }} />
                </span>
              </div>
            </div>
          </div>
        ...
        {/* Composer */}
        <div className="shrink-0 shadow-[inset_0_1px_0_0_hsl(var(--hairline))] bg-surface px-6 py-3">
          <div className="rounded-md bg-elevated/60 shadow-hairline">
            ...
            />
            <div className="flex items-center gap-1 shadow-[inset_0_1px_0_0_hsl(var(--hairline))] px-2 py-1.5">
              ...
              <Button variant="ghost" size="xs" className="gap-1 ml-1">
      ...
      {/* Context panel */}
      <aside className="hidden w-[320px] shrink-0 flex-col lg:flex shadow-[inset_1px_0_0_0_hsl(var(--hairline))] overflow-y-auto">
        {lead && (
          <>
            ...
            <div className="px-5 py-5 shadow-hairline-b">
              ...
              <SidebarSection title="Valor">
                <div className="font-metric text-base text-ink tabular-nums">
...
function SuggestionCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="group rounded-md bg-elevated/40 p-3 shadow-hairline hover:bg-elevated transition-colors cursor-pointer">
      <div className="flex items-start gap-2">
        <Sparkles className="mt-0.5 h-3 w-3 text-accent shrink-0" />
        <div className="min-w-0 flex-1">
          <div className="text-sm font-medium text-ink">{title}</div>
          <div className="mt-0.5 text-xs text-ink-muted leading-snug">{desc}</div>
        </div>
      </div>
    </div>
  );
