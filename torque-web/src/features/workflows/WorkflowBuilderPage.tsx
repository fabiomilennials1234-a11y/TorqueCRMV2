...
import {
  Play, Save, Undo2, Redo2, Grid3x3, Sparkles,
  Zap, GitBranch, Clock, Send, Tag, ArrowRight,
  MessagesSquare, Database, Webhook, UserPlus,
} from "lucide-react";
...
export function WorkflowBuilderPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] min-h-0 flex-col">
      {/* Top bar */}
      <div className="shrink-0 flex items-center justify-between gap-4 px-8 py-4 shadow-hairline-b bg-bg">
        <div>
      ...
      <div className="grid flex-1 min-h-0 grid-cols-[260px_1fr_320px]">
        ...
        <aside className="min-h-0 overflow-y-auto bg-surface p-4 shadow-[inset_-1px_0_0_0_hsl(var(--hairline))]">
          ...
          <PaletteGroup label="Gatilhos" items={[
          ...
          <PaletteGroup label="Controle" items={[
          ...
          <PaletteGroup label="Ações" items={[
        ...
        <main className="relative min-h-0 overflow-auto bg-bg">
          ...
          />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_40%,hsl(var(--bg))_100%)] pointer-events-none" />
          <div className="relative min-w-[1100px] min-h-[600px] p-8">
...
function PaletteGroup({
  ...
  return (
    <div className="mb-5">
      <div className="mb-2 text-2xs uppercase tracking-[0.12em] text-ink-dim">
        {label}
      </div>
      <div className="space-y-1">
        {items.map((it) => {
          ...
          return (
            <button
              key={it.label}
              className="group flex w-full items-center gap-2 rounded-sm bg-elevated/50 px-2.5 py-2 text-[0.8125rem] text-ink-muted shadow-hairline hover:bg-elevated hover:text-ink transition-colors cursor-grab active:cursor-grabbing"
            >
              <Icon className="h-3.5 w-3.5 text-ink-dim group-hover:text-accent" strokeWidth={1.75} />
...
function WorkflowNode({
  kind, title, note, x, y, active, onClick,
}: {
  id: string;
  kind: NodeKind;
  title: string;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-2xs font-medium uppercase tracking-[0.12em]" style={{ color: meta.color }}>
          {meta.label}
        </span>
      </div>
      <div className="mt-2 text-[0.875rem] font-medium text-ink leading-tight">
        {title}
      </div>
      {note && (
        <div className="mt-1 text-2xs text-ink-dim">{note}</div>
      )}
      {/* IO pins */}
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-display text-lg tracking-tightest text-ink">
          Inspector
        </h3>
      ...
      <div className="mb-5 rounded-md bg-elevated/60 p-3 shadow-hairline">
        ...
        <div className="mt-1 font-metric text-xs text-ink">{nodeId}</div>
      </div>

      <FieldGroup label="Avaliar">
        <select className="h-9 w-full rounded-md bg-elevated/60 px-3 text-sm text-ink shadow-hairline appearance-none">
        ...
        <div className="flex gap-1">
          {["=", "≥", ">", "<", "≠"].map((op) => (
            <button
              key={op}
              className={cn(
                "flex h-8 w-8 items-center justify-center rounded-sm font-metric text-sm",
                op === "≥"
      ...
      <FieldGroup label="Valor">
        <input
          defaultValue="60"
          className="h-9 w-full rounded-md bg-elevated/60 px-3 font-metric text-sm text-ink shadow-hairline focus-visible:shadow-[inset_0_0_0_1px_hsl(var(--accent))]"
        />
      </FieldGroup>
      ...
      <div className="mt-6 rounded-md bg-accent/5 p-3 shadow-[inset_0_0_0_1px_hsl(var(--accent)/0.2)]">
        ...
        <p className="text-xs text-ink leading-relaxed">
          Leads com score entre 60 e 70 convertem 18% mais quando recebem uma
          mensagem de humanização antes do handoff. Quer adicionar esse passo?
        </p>
